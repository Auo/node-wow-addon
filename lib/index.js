...
module.exports = function(installationPath) {
  ...
  const deleteAddon = function deleteAddon(name, cb) {
    const config = jsonfile.readFileSync(path.join(installationPath, 'addons.json'))
    console.log(config, ' config, before removing')
  ...
  const installAddon = function installAddon(info, cb) {
    ...
    const s = fs.createWriteStream(path.join(installationPath, 'temp-addon.zip'))

    const stream = request(info.downloadLink)
    .on('error', (err) => {
      return cb(err)
    }).pipe(s)

    stream.on('finish', function(response) {
      const zip = new AdmZip(path.join(installationPath, 'temp-addon.zip'))
      ...
      if(foldersToRemove.length > 0) {
        // const glob = '{' + foldersToRemove.map(f=> { return path.join(installationPath,f) }).join() + '}'
        ...
        rimraf(glob, (err) => {
          ...
          fs.unlinkSync(path.join(installationPath,'temp-addon.zip'))
