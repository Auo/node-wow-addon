...
module.exports = function(installationPath) {
  ...
  const installAddon = function installAddon(info, cb) {
    const fileEnding = info.downloadLink.substring(info.downloadLink.lastIndexOf('.'), info.downloadLink.length)
    ...
    stream.on('finish', function() {
      if(fileEnding == '.zip') {
        ...
        const rootFolders = folders.map(entry => {return entry.substring(0, entry.indexOf('/')) })
        .filter((value, index, self) => { return self.indexOf(value) === index })
