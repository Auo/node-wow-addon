...
module.exports = {
  _parseDownloads(downloads) {
    downloads = downloads.replace('Downloads', '').trim()

    if (downloads.indexOf('M') != -1) {
      return parseInt(downloads.substring(0, downloads.length - 1)) * 1000000
    } else if (downloads.indexOf('K') != -1) {
      return parseInt(downloads.substring(0, downloads.length - 1)) * 10000
    } else {
  ...
  _getSearchResults: function (html) {
    ...
    $('div.my-2').each((i, element) => {
      ...
      const downloads = this._parseDownloads($('span.mr-2').first().text())

      const image = $('img.mx-auto', element).attr('href');
      const categories = []
  ...
  getCategories: function (cb) {
    ...
    request(url, (err, response) => {
      ...
      elements.each((i, element) => {
        ...
        if (!hasChildren) {
          name = $('span.whitespace-no-wrap', element).text().replace(/\n/g, '').trim()
        } else {
          const children = $('div.px-2', element)
